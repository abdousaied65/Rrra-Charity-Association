...
<script>
    $(document).ready(function () {
        ...
        $('#example-table tfoot tr th:nth-child(6)').html('<select name="order_type_id" required class="form-control"><option value="">اختر نوع الطلب </option>@foreach($order_types as $order_type)<option value="{{$order_type->order_type}}">{{$order_type->order_type}}</option>@endforeach</select>');
